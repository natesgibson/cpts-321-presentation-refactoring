...
namespace Refactoring_Code_Demo
{
    ...
    internal class Boss
    {
        ...
        {
            if (targetPlayer.State == Player.PlayerState.Idle) return Attack1();
            if (targetPlayer.State == Player.PlayerState.Walking) return Attack2();
            if (targetPlayer.State == Player.PlayerState.Running) return Attack3();
            if (targetPlayer.State == Player.PlayerState.Swimming || targetPlayer.State == Player.PlayerState.InAir) return Attack4();
            return false;
        }
